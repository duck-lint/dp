...
Clues and structural analysis update immediately as you edit. Exact uniqueness
is computed asynchronously after editing pauses, so `pending…` or `checking…`
is expected while the worker evaluates the current candidate.

During play, the progress panel includes a small set of local achievements
derived from completion and streak records. They are intentionally browser-local
and require no account or server connection.
